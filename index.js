...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:ZanxX\n' // GANTI NAMA LU
            + 'ORG:Rinn;\n'
            + 'TEL;type=CELL;type=VOICE;waid=628810239622630+62 881-0239-62263\n' // GANTI NOMOR LU
            + 'END:VCARD'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' Rinn'))
})
	...
	baby.on('connecting', () => {
		start('2', 'Rinn Connecting...')
	})
	baby.on('open', () => {
		success('2', 'Rinn Connected')
	})
...
*/
				...
				if (isBanned) return reply(nad.baned())
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Rinn', authorname)} ${ran} -o ${ran}`, async (error) => {
			...
			run = `「 *RUNTIME* 」\n${kyun(uptime)}`
			baby.sendMessage(from, run, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: ` 𝐁𝐎T 𝐕𝐄𝐑𝐈𝐅𝐈𝐄𝐃` }}})
			break
				case 'nulis':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis Rinn baik hati`)
				nul = body.slice(7)
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 Rinn baik hati`)
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 Rinnn baik hati`)
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub Rinn & Hub`)
				reply(nad.wait())
		...
		case 'textlight':
		    ...
		    break
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, `Kode bahasanya mana kak? contoh : ${prefix}tts id Halo Rinn`, text, {quoted: mek})
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata Rinn`
...
*/
                ...
                addKoinUser('62881023962263@s.whatsapp.net', fee)
